    ...
    def keep_seconds(self, start, end):
        if self.framework == "arkit":
            ...
            for key, value in self.data.items():
                total_duration = len(value) / self.fps
                break
        return self.trim_seconds(start, total_duration-end)
...
## this one to load .pkl object (data)
...
def cbta(gaitdata):
    ...
    ## fig_dir this line is modified
    
    fig_dir = "C:\\Users\PC\Desktop\plots\\" 
...
def print_analysis_HSTO(HS, TO, use_gaitdata: bool, gaitdata = GaitData("none", 0), fps=50):
    ...
    for side in Side:
        ...
        stride_pairs[side] = find_pairs_strict(HS[side], HS[side])
    ...
    csvfile = open("C:\\Users\PC\Desktop\gaitevent_fortest\gaitanalysis.csv", 'w', newline='')       ##this line is modifiled,mistakes?
    ...
    for side in Side:
        ...
        for i in range(len(stride_pairs[side])):
            ...
            csvwriter.writerow(['Double support', str(double_support)])
            ...
            csvwriter.writerow(['Single support', str(single_support_time)])
            # csvwriter.writerow(['Step width', str(step_width)])
            for k in range(len(step_pairs[side])):
                if is_pair_inside_pair(step_pairs[side][k], stride_pairs[side][i]):
                    if use_gaitdata:
                        csvwriter.writerow(['Step length', str(step_lengths[side][k])])
                    csvwriter.writerow(['Step time', str(step_durations[side][k])])
            if use_gaitdata:
                csvwriter.writerow(['Stride length', str(stride_lengths[side][i])])
            csvwriter.writerow(['Stride time', str(stride_durations[side][i])])
            if use_gaitdata:
                csvwriter.writerow(['Walking speed', str(walking_speed)])
            csvwriter.writerow(['Cadence', str(cadence * 60)])
    ...
    if use_gaitdata:
        ...
        for side in Side:
            for i in range(len(time_axis)):
                knee_flexion[side].append(find_flexion_deg(gaitdata.walk_direction,filt_hip[side][i], filt_knee[side][i], filt_ank[side][i]))
                if gaitdata.framework != 'vicon':
                    hip_flexion[side].append(-find_flexion_deg(gaitdata.walk_direction, filt_midshoulder[i], filt_hip[side][i], filt_knee[side][i]))
            ...
            for i in range(len(stride_pairs[side])):
                ...
                # temp_title = gaitdata.framework + " " + side.name + " Sole to Floor Angle " + str(i+1)
                # plt.figure(temp_title)
                # plt.plot(time_axis[start:end], sole_to_floor[side][start:end], line_style[side])
                # plt.gcf().savefig(fig_dir + temp_title)
...
def find_flexion_deg(face_direction: Side, upper_joint, middle_joint, lower_joint):
    ...
    vector_upper = upper_joint[0:2] - middle_joint[0:2]
    vector_lower = lower_joint[0:2] - middle_joint[0:2]
